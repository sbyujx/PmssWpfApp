...
namespace PMSS.SqlDataOutput
{
    public class FetchRiverRecord
    {
        ...
        public FetchRiverRecord()
        {
            riverList = new List<RiverHydrologyRecord>();
        ...
        public List<RiverHydrologyRecord> RiverList
        {
            get
            {
                return riverList;
        ...
        public void SaveToTxTFile(string fileName)
        {
            ...
            ls.Add(str);
            foreach (RiverHydrologyRecord rhr in riverList)
            {
                str = "";
                ...
                str += rhr.Flow.ToString("F2") + "\t";
                str += rhr.Time.ToString() + "\t";
                str += rhr.WarningLevel.ToString("F2") + "\t";
                str += rhr.SafetyLevel.ToString("F2") + "\t";
        ...
        public void SaveToExcelFile(string fileName)
        {
            ...
            try
            {
                ...
                oSheet.Cells[1, 7] = "流量";
                oSheet.Cells[1, 8] = "时间\t";
                oSheet.Cells[1, 9] = "警戒水位";
                oSheet.Cells[1, 10] = "保证水位";
                oSheet.Cells[1, 11] = "经度";
                oSheet.Cells[1, 12] = "纬度";
                ...
                foreach (RiverHydrologyRecord rhr in riverList)
                {
                    ...
                    oSheet.Cells[row, 7] = rhr.Flow.ToString("F2");
                    oSheet.Cells[row, 8] = rhr.Time.ToString();
                    oSheet.Cells[row, 9] = rhr.WarningLevel.ToString("F2");
                    oSheet.Cells[row, 10] = rhr.SafetyLevel.ToString("F2");
                    oSheet.Cells[row, 11] = rhr.Longitude.ToString("F2");
                    oSheet.Cells[row, 12] = rhr.Latitude.ToString("F2");
                    row++;
                }

                oRng = oSheet.Range[oSheet.Cells[1, 1], oSheet.Cells[row, 12]];
